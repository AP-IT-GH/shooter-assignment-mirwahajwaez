...
public class Damage : MonoBehaviour {
	...
	{
		if (damageOnTrigger) {
			if (this.CompareTag("Missile") && collision.gameObject.CompareTag("Player"))	// if the player got hit with it's own bullets, ignore it
				return;
	...
	{	
		if (damageOnCollision) {
			if (CompareTag("Missile") && collision.gameObject.CompareTag("Player"))	// if the player got hit with it's own bullets, ignore it
				return;
	...
	{
		if (continuousDamage) {
			if (collision.gameObject.CompareTag("Player") && collision.gameObject.GetComponent<Health> () != null) {	// is only triggered if whatever it hits is the player
